import { Card, Spinner, CardContent, Tooltip, Button, /*  styled,  */ Inline, Text, Chevron, IconWrapper } from 'junoblocks';
    ...
    <Card variant="secondary" disabled css={{ padding: '$2' }}>
      <CardContent css={{ margin: '$4 $3' }} size="medium">
        <Inline justifyContent={'space-between'}>
          ...
          <Button variant="ghost" onClick={() => hideICAInfo((showICAInfo) => !showICAInfo)} >  {showICAInfo ? <IconWrapper size="medium"
            rotation="90deg"
            color="tertiary"
            icon={<Chevron />} />
            :
      ...
      {showICAInfo && <CardContent>
        <Text variant="legend"> Address </Text><Text css={{ padding: '$4' }} variant="caption"> {icaAddress}</Text>
        {!isIcaBalanceLoading && <> <Text variant="legend"> Balance  </Text> <Text css={{ padding: '$4' }} variant="caption"> {icaBalance} {chainSymbol}</Text></>}
        <Text variant="legend"> Grants</Text>
        {!isAuthzGrantsLoading && icaAuthzGrants && icaAuthzGrants[0] && icaAuthzGrants[0].msgTypeUrl ? <>{icaAuthzGrants.map((grant) => <Text css={{ padding: '$4' }} variant="caption"> Has grant for message type: '{grant.msgTypeUrl}' {/* that expires in {(relativeTime(grant.expiration.seconds.toNumber() * 1000))}  */}</Text>
        )}</> : (!icaAuthzGrants && <Text css={{ padding: '$4' }} variant="caption">No AuthZ grants for specified message types (yet)</Text>)}
        {!shouldDisableAuthzGrantButton && <>
          ...
          <Card variant="secondary" disabled css={{ padding: '$4', margin: '$4' }}>
            <CardContent>
              ...
              ><Text variant="legend"> Top up</Text></Tooltip>
              <Row>
                <Text variant="caption"><StyledInput step=".01"
                  placeholder="0.00" type="number"
                  value={feeFundsHostChain}
                  onChange={({ target: { value } }) => setFeeFundsHostChain(value)} />{chainSymbol}
                </Text>
                <Button css={{ margin: '$2' }}
                  variant="secondary"
                  size="small"
