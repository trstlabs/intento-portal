...
export TX_HASH=$(
    docker exec test-localtrst-1 trstd tx compute instantiate $factory_code_id '{"pair_code_id": '$pair_code_id', "pair_code_hash": '$pair_code_hash', "token_code_id": '$token_code_id', "token_code_hash": '$token_code_hash', "fee_auto_msg": 0, "fee_recurring_auto_msg":0,"prng_seed": "YWE="}' --contract_id BackSwap --from $deployer_name -b sync --gas 3000000 --fees 700utrst -y |
    jq -r .txhash
)
wait_for_tx "$TX_HASH" "Waiting for tx to finish on-chain..."
docker exec test-localtrst-1 trstd q compute tx $TX_HASH
...
export TX_HASH=$(
    docker exec test-localtrst-1 trstd tx compute instantiate $keyring_code_id '{"prng_seed":"YWE="}' --contract_id Keyring --from $deployer_name  --output json --gas 3000000 --fees 700utrst -y |
    jq -r .txhash
)
wait_for_tx "$TX_HASH" "Waiting for tx to finish on-chain..."
docker exec test-localtrst-1 trstd q compute tx $TX_HASH
...
export TX_HASH=$(
    docker exec test-localtrst-1 trstd tx compute instantiate $token_code_id '{"admin": "'$deployer_address'", "symbol": "PETH", "decimals": 18, "initial_balances": [{"address": "'$deployer_address'", "amount": "100000000000000000000000"}],  "prng_seed": "YWE=", "name": "test", "keyring": {"contract": '$keyring_contract', "code_hash":'$keyring_code_hash'}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst --gas 1500000 --contract_id pETH -b block -y |
    jq -r .txhash
)
wait_for_tx "$TX_HASH" "Waiting for tx to finish on-chain..."
docker exec test-localtrst-1 trstd q compute tx $TX_HASH
...
export TX_HASH=$(
    docker exec test-localtrst-1 trstd tx compute instantiate $token_code_id '{"admin": "'$deployer_address'", "symbol": "PATOM", "decimals": 18, "initial_balances": [{"address": "'$deployer_address'", "amount": "100000000000000000000000"}], "prng_seed": "YWE=", "name": "test", "keyring": {"contract": '$keyring_contract', "code_hash":'$keyring_code_hash'}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst --gas 1500000 --contract_id pATOM -b block -y |
    jq -r .txhash
)
wait_for_tx "$TX_HASH" "Waiting for tx to finish on-chain..."
docker exec test-localtrst-1 trstd q compute tx $TX_HASH
...
export TX_HASH=$(
    docker exec test-localtrst-1 trstd tx compute instantiate $token_code_id '{"admin": "'$deployer_address'", "symbol": "PTRST", "decimals": 6, "initial_balances": [{"address": "'$deployer_address'", "amount": "200000000000000000000000"}], "prng_seed": "YWE=", "name": "test", "keyring": {"contract": '$keyring_contract', "code_hash":'$keyring_code_hash'}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst --gas 1500000 --contract_id pTRST -b block -y |
    jq -r .txhash
)
wait_for_tx "$TX_HASH" "Waiting for tx to finish on-chain..."
docker exec test-localtrst-1 trstd q compute tx $TX_HASH
...
echo "pTRST address: '$trst_addr'"

echo "Deploying pOSMO..."

export TX_HASH=$(
    docker exec test-localtrst-1 trstd tx compute instantiate $token_code_id '{"admin": "'$deployer_address'", "symbol": "POSMO", "decimals": 8, "initial_balances": [{"address": "'$deployer_address'", "amount": "10000000000000"}], "prng_seed": "YWE=", "name": "test", "keyring": {"contract": '$keyring_contract', "code_hash":'$keyring_code_hash' }}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst --gas 1500000 --contract_id pOSMO -b block -y |
    jq -r .txhash
)
wait_for_tx "$TX_HASH" "Waiting for tx to finish on-chain..."
docker exec test-localtrst-1 trstd q compute tx $TX_HASH
...
echo "pATOM address: '$osmo_addr'"

echo "Deploying router..."

export TX_HASH=$(
    docker exec test-localtrst-1 trstd tx compute instantiate $router_code_id '{"register_tokens":[{"address":'$trst_addr',"code_hash":'$token_code_hash'},{"address":'$eth_addr',"code_hash":'$token_code_hash'},{"address":'$atom_addr',"code_hash":'$token_code_hash'}, {"address":'$osmo_addr',"code_hash":'$token_code_hash'}]}' --contract_id router --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y |
    jq -r .txhash
)
wait_for_tx "$TX_HASH" "Waiting for tx to finish on-chain..."
docker exec test-localtrst-1 trstd q compute tx $TX_HASH
...
pair_contract_eth_ptrst=$(trstd query compute list-contracts-by-code $pair_code_id | jq '.[-1].address')
echo "pETH/pTRST Pair contract address: '$pair_contract_eth_ptrst'"

docker exec test-localtrst-1 trstd tx compute execute $(echo "$eth_addr" | tr -d '"') '{"increase_allowance": {"spender": '$pair_contract_eth_ptrst', "amount": "10000000000000000000"}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$trst_addr" | tr -d '"') '{"increase_allowance": {"spender": '$pair_contract_eth_ptrst', "amount": "1000000000000000000000"}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$pair_contract_eth_ptrst" | tr -d '"') '{"provide_liquidity": {"assets": [{"info": {"token": {"contract_addr": '$trst_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}, "amount": "1000000000000000000000"}, {"info": {"token": {"contract_addr": '$eth_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}, "amount": "10000000000000000000"}]}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y -b block

echo "Creating pATOM/pTRST pair..."
...
pair_contract_atom_ptrst=$(trstd query compute list-contracts-by-code $pair_code_id | jq '.[-1].address')
echo "pATOM/pTRST Pair contract address: '$pair_contract_atom_ptrst'"

docker exec test-localtrst-1 trstd tx compute execute $(echo "$atom_addr" | tr -d '"') '{"increase_allowance": {"spender": '$pair_contract_atom_ptrst', "amount": "100000000000"}}' -b block -y --from $deployer_name  -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$trst_addr" | tr -d '"') '{"increase_allowance": {"spender": '$pair_contract_atom_ptrst', "amount": "200000000000"}}' -b block -y --from $deployer_name  -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$pair_contract_atom_ptrst" | tr -d '"') '{"provide_liquidity": {"assets": [{"info": {"token": {"contract_addr": '$trst_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}, "amount": "200000000000"}, {"info": {"token": {"contract_addr": '$atom_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}, "amount": "100000000000"}]}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y -b block


echo "Creating pOSMO/pTRST pair..."
...
pair_contract_osmo_ptrst=$(trstd query compute list-contracts-by-code $pair_code_id | jq '.[-1].address')
echo "pATOM/pTRST Pair contract address: '$pair_contract_osmo_ptrst'"

docker exec test-localtrst-1 trstd tx compute execute $(echo "$osmo_addr" | tr -d '"') '{"increase_allowance": {"spender": '$pair_contract_osmo_ptrst', "amount": "100000000000"}}' -b block -y --from $deployer_name  -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$trst_addr" | tr -d '"') '{"increase_allowance": {"spender": '$pair_contract_osmo_ptrst', "amount": "200000000000"}}' -b block -y --from $deployer_name  -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$pair_contract_osmo_ptrst" | tr -d '"') '{"provide_liquidity": {"assets": [{"info": {"token": {"contract_addr": '$trst_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}, "amount": "200000000000"}, {"info": {"token": {"contract_addr": '$osmo_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}, "amount": "10000000000"}]}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y -b block

echo "Creating TRST/pTRST pair..."

docker exec test-localtrst-1 trstd tx compute execute --contract_id BackSwap '{"create_pair": {"asset_infos": [{"native_token": {"denom": "utrst"}},{"token": {"contract_addr": '$trst_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}],"keyring_address": '$keyring_contract', "keyring_code_hash": '$keyring_code_hash'}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y -b block

pair_contract_ptrst_trst=$(trstd query compute list-contracts-by-code $pair_code_id | jq '.[-1].address')
echo "TRST/pTRST Pair contract address: '$pair_contract_ptrst_trst'"


docker exec test-localtrst-1 trstd tx compute execute $(echo "$trst_addr" | tr -d '"') '{"increase_allowance": {"spender": '$pair_contract_ptrst_trst', "amount": "5000000000"}}' -b block -y   --from $deployer_name -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$pair_contract_ptrst_trst" | tr -d '"') '{"provide_liquidity": {"assets": [{"info": {"native_token": {"denom": "utrst"}}, "amount": "5000000000"}, {"info": {"token": {"contract_addr": '$trst_addr', "token_code_hash": '$token_code_hash', "viewing_key": ""}}, "amount": "5000000000"}]}}' --from $deployer_name -b block   --gas 3000000 --fees 500utrst --amount 5000000000utrst -y --gas 1500000 -b block

trstd tx bank send $deployer_name $(echo "$trst_addr" | tr -d '"') 100000000utrst -y -b block -b block   --gas 3000000 --fees 500utrst
docker exec test-localtrst-1 trstd tx compute execute $(echo "$eth_addr" | tr -d '"') '{"transfer":{"recipient":'$factory_contract',"amount":"1000000000000000000000"}}' -y -b block   --from "$deployer_name" -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$atom_addr" | tr -d '"') '{"transfer":{"recipient":'$factory_contract',"amount":"100000000000"}}' -y -b block   --from "$deployer_name" -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$trst_addr" | tr -d '"') '{"transfer":{"recipient":'$factory_contract',"amount":"1000000000"}}' -y -b block   --from "$deployer_name" -b block   --gas 3000000 --fees 500utrst -y -b block
docker exec test-localtrst-1 trstd tx compute execute $(echo "$osmo_addr" | tr -d '"') '{"transfer":{"recipient":'$factory_contract',"amount":"1000000000"}}' -y -b block   --from "$deployer_name" -b block   --gas 3000000 --fees 500utrst -y -b block
...
echo Pairs:
docker exec test-localtrst-1 trstd q compute query $(echo "$factory_contract" | tr -d '"') '{"pairs":{}}' | jq -c .pairs
